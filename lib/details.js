...
function* getDetail(urls) {
  ...
  function handleDone($tr,obj,$,done,resolve){
    $tr.each((idx, tr) => {
      if (idx === 0) return;
      Object.assign(obj,
        getTradingRecord($(tr)));
        ...
        if (error) {
          ...
          if ($tr.length < 2) {
            ...
            if (match && match.length) {
              const id = match[1];
              request.post({ url: `http://112.74.98.194/products/${id}/get_commodity_prices` }, (err, httpResponse, body) => {
